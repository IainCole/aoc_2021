...
WITH numbered_lines as (
    ...
    SELECT DISTINCT row_num, number, bnum.board_num
    ...
    SELECT r.row_num, r.board_num, r.number
    FROM results r
    WHERE r.board_num NOT IN (select board_num FROM results r2 where r2.row_num < r.row_num)
), loser as (
    SELECT *
    FROM win_order
...
SELECT l.board_num, SUM(b.number) * l.number as solution
FROM loser l
INNER JOIN boards b on l.board_num = b.board_num AND b.number NOT IN (SELECT number FROM ordered_numbers WHERE row_num <= l.row_num)
GROUP BY l.board_num, l.number